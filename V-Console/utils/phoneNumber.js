...
const LANDLINE_PREFIX = '296,254,209,204,291,222,275,256,274,271,252,290,292,206,236,262,261,215,251,277,269,226,24,239,220,225,293,28,221,258,297,260,213,263,205,214,272,228,238,259,229,257,232,235,255,203,233,299,212,276,227,208,237,234,273,294,207,270,216'

export const formatPhoneNumber = phoneNumber => {
  let converted = phoneNumber
  if (!_.isString(converted) || blank(converted)) {
    return null
  }
  ...
  if (!/^84\d{7}$/.test(converted)) {
    converted = converted.replace(/^84/, '')
  }

  // Convert prefix
  _.some(DIGIT11_TO_10_PREFIX, (to, from) => {
...
export const isMobileNumber = phoneNumber => {
  ...
  return /^0\d{9,10}$/.test(formatPhoneNumber(phoneNumber))
}
