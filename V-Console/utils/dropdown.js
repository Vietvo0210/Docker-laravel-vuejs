...
export const convert2DropdownOptions = enums => {
  return _.map(enums, (label, value) => ({ label, value }))
}
      ...
      return Promise.resolve()
        ...
        .then(() => {
          if (dropdowns.length === 1) {
            ...
            if (belongsWithConditions
              && notEmpty(conditions)
              && _.every(conditions, blank)
            ) {
              return []
            }
          }
        ...
        .then(options => {
          dropdowns.forEach(dropdown => {
            ...
            if (_.isArray(optionValues)) {
              ...
              const newValue = optionValues.filter(item => {
